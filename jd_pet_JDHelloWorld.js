...
let jdNotify = false, runTimesErr = '', runTimesErrCount = 0;//是否关闭通知，false打开通知推送，true关闭通知推送
...
function autoInsert() {
  return new Promise((resolve) => {
    $.get({
      url: `https://api.jdsharecode.xyz/api/autoInsert/pet?uid=${process.env.TG_USER_ID ?? ''}&token=${encodeURIComponent(process.env.TG_USER_TOKEN_HW) ?? ''}&sharecode=${$.petInfo.shareCode}`
    }, (err, resp, data) => {
      try {
        if (err) {
