...
function getMoney_dwSource_1(_key, sceneList) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        `user/GetMoney`,
        `dwSceneId=${_key}&strEmployeeId=undefined&dwSource=1`
      ),
      async (err, resp, data) => {
        try {
...
function getMoney_dwSource_2(_key, sceneList, key) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        `user/GetMoney`,
        `dwSceneId=${_key}&strEmployeeId=${key}&dwSource=2`
      ),
      async (err, resp, data) => {
        try {
...
function getMoney_dwSource_3(_key, sceneList) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        `user/GetMoney`,
        `dwSceneId=${_key}&strEmployeeId=&dwSource=3&strPgtimestamp=${$.currentToken["timestamp"]}&strPhoneID=${$.currentToken["phoneid"]}&strPgUUNum=${$.currentToken["farm_jstoken"]}`
      ),
      async (err, resp, data) => {
        try {
...
function getAdvEmployee(_key) {
  $.get(
    taskUrl(
      `user/GetAdvEmployee`,
      `dwSenceId=${_key}&dwIsSlave=0&_stk=_cfd_t%2CbizCode%2CdwEnv%2CdwIsSlave%2CdwSenceId%2Cptag%2Csource%2CstrZone`
    ),
    async (err, resp, data) => {
      try {
        const {
...
function advEmployeeAward(_key, strSceneName) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        `user/AdvEmployeeAward`,
        `dwSenceId=${_key}&_stk=_cfd_t%2CbizCode%2CdwEnv%2CdwSenceId%2Cptag%2Csource%2CstrZone`
      ),
      async (err, resp, data) => {
        try {
          $.log(data);
...
function promotionAward() {
  $.get(
    taskUrl(
      `user/PromotionAward`,
      `_stk=_cfd_t%2CbizCode%2CdwEnv%2Cptag%2Csource%2CstrZone`
    ),
    async (err, resp, data) => {
      try {
...
function friendCircle() {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(`user/FriendCircle`, `dwPageIndex=1&dwPageSize=20`),
      async (err, resp, data) => {
        try {
          //$.log(`\n好友圈列表\n${data}`);
          const { MomentList = [], iRet, sErrMsg, strShareId } = JSON.parse(
            data
          );
...
function queryFriendIsland(strShareId) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(`user/QueryFriendIsland`, `strShareId=${strShareId}&sceneval=2`),
      async (err, resp, data) => {
        try {
          //$.log(`\n获取好友信息\n${data}`);
          const {
...
function stealMoney(strShareId, sceneId, strFriendNick, strSceneName) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        `user/StealMoney`,
        `strFriendId=${strShareId}&dwSceneId=${sceneId}&sceneval=2`
      ),
      async (err, resp, data) => {
        try {
          //$.log(data);
    ...
    switch (taskType) {
      case 0: //日常任务
        ...
        $.get(taskListUrl(`Award`, `taskId=${taskId}`), (err, resp, data) => {
          try {
            const { msg, ret, data: { prizeInfo = "" } = {} } = JSON.parse(
              data
            );
            let str = "";
      ...
      case 1: //成就奖励
        ...
        $.get(
          taskUrl(`consume/AchieveAward`, `strTaskIndex=${strTaskIndex}`),
          (err, resp, data) => {
            try {
      ...
      async (err, resp, _data) => {
        try {
          ...
          $.get(
            taskUrl(
              "user/JoinScene",
              `strPgtimestamp=${$.currentToken["timestamp"]}&strPhoneID=${
                $.currentToken["phoneid"]
              }&strPgUUNum=${
                $.currentToken["farm_jstoken"]
              }&strShareId=${escape(
                data.value
              )}&dwSceneId=${sceneId}&dwType=2&_stk=_cfd_t%2CbizCode%2CdwEnv%2CdwSceneId%2CdwType%2Cptag%2Csource%2CstrPgUUNum%2CstrPgtimestamp%2CstrPhoneID%2CstrShareId%2CstrZone`
            ),
            async (err, resp, data) => {
              try {
                const { sErrMsg, data: { rewardMoney = 0 } = {} } = JSON.parse(
                  data
                );
                $.log(
                  `\n【👫🏻超级助力】超级助力：${sErrMsg}\n${
...
function openGroup() {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(`user/OpenGroup`, `dwIsNewUser=${$.info.dwIsNewUser}`),
      async (err, resp, data) => {
        try {
      ...
      (err, resp, _data) => {
        try {
          ...
          $.get(
            taskUrl(
              `user/JoinGroup`,
              `strGroupId=${data.value}&dwIsNewUser=${$.info.dwIsNewUser}&pgtimestamp=${$.currentToken["timestamp"]}&phoneID=${$.currentToken["phoneid"]}&pgUUNum=${$.currentToken["farm_jstoken"]}&_stk=_cfd_t%2CbizCode%2CdwEnv%2CdwIsNewUser%2CpgUUNum%2Cpgtimestamp%2CphoneID%2Cptag%2Csource%2CstrGroupId%2CstrZone`
            ),
            (err, resp, data) => {
              try {
                const { sErrMsg } = JSON.parse(data);
