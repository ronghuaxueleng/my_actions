...
function autoInsert() {
  return new Promise((resolve) => {
    $.get({
      url: `https://api.jdsharecode.xyz/api/autoInsert/bean?uid=${process.env.TG_USER_ID ?? ''}&token=${encodeURIComponent(process.env.TG_USER_TOKEN_HW) ?? ''}&sharecode=${$.myPlantUuid}`
    }, (err, resp, data) => {
      try {
        if (err) {
