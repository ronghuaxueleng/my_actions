/* 
#joy总动员
30 10 * * * https://raw.githubusercontent.com/Shineslee/JD_Script/master/dockerdiy/jd_friend.js, tag=joy总动员, enabled=true
*/
...
function addFriend(body) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl("jdf_updateFriendshipStatus", body),
      (err, resp, data) => {
        try {
          if (err) {
...
function getFriendList() {
  return new Promise((resolve) => {
    $.get(
      taskGetUrl("getFriendList", { clientType: "jdApp" }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
        ...
        try {
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data.code === "0") {
                console.log(
                  `与好友【${unionPin}】匹配成功，匹配程度：${data.data.intimacy.total}`
                );
...
function openBox(unionPin, type = 1) {
  return new Promise((resolve) => {
    $.get(
      taskGetUrl("openBox", { pin: unionPin, clientType: "jdApp", type: type }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            ...
            if (safeGet(data)) {
              ...
              if (data.code === "0") {
                console.log(
                  `打开宝箱成功，获得${data.data.bean ? data.data.bean : 0}京豆`
                );
...
function taskGetUrl(function_id, body) {
  ...
  return {
    url: `https://api.m.jd.com/?appid=joy-friend&functionId=${function_id}&body=${escape(
      JSON.stringify(body)
    )}`,
    headers: {
      Cookie: cookie,
