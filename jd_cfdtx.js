...
requestAlgo();
for (let ix = 0; ix < cookiesArr.length; ix++) {
  !(async (i) => {
    if (cookiesArr[i]) {
      ...
      let info = {}
      let token = await getJxToken()
      await cfd(cookie, token, info, index, nickName, UserName);
    }
    if (allMessage) {
...
async function cfd(cookie, token, info, index, nickName, UserName) {
  try {
    let beginInfo = await getUserInfo(cookie, info, index, UserName, false);
    if (beginInfo.Fund.ddwFundTargTm === 0) {
      console.log(`还未开通活动，请先开通\n`)
      return
    }
    await cashOutQuali(cookie, token)
    await userCashOutState(cookie, token, info, index, UserName)
    await showMsg(index, nickName, UserName)
  } catch (e) {
    $.logErr(e)
  }
}
...
// 提现
function cashOutQuali(cookie, token) {
  return new Promise((resolve) => {
    $.get(taskUrl(`user/CashOutQuali`, cookie, `strPgUUNum=${token['farm_jstoken']}&strPgtimestamp=${token['timestamp']}&strPhoneID=${token['phoneid']}`), (err, resp, data) => {
      try {
        if (err) {
...
async function userCashOutState(cookie, token, info) {
  return new Promise(async (resolve) => {
    $.get(taskUrl(`user/UserCashOutState`, cookie), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          ...
          if (data.dwTodayIsCashOut !== 1) {
            if (data.ddwUsrTodayGetRich >= data.ddwTodayTargetUnLockRich) {
              for (let key of Object.keys(data.UsrCurrCashList).reverse()) {
                ...
                if (vo.dwDefault === 1) {
                  let cashOutRes = await cashOut(vo.ddwMoney, vo.ddwPaperMoney, cookie, token)
                  if (cashOutRes.iRet === 0) {
                    $.money = vo.ddwMoney / 100
                    console.log(`提现成功获得：${$.money}元`)
                  } else {
                    await userCashOutState(cookie, token, info)
                  }
                }
              }
            } else {
              ...
              for(let key of Object.keys(info.buildInfo.buildList)) {
                ...
                let getBuildInfoRes = await getBuildInfo(cookie, body)
                let buildNmae;
                ...
                if(getBuildInfoRes.dwCanLvlUp > 0 && info.ddwCoinBalance >= getBuildInfoRes.ddwNextLvlCostCoin) {
                  ...
                  let buildLvlUpRes = await buildLvlUp(cookie, body)
...
function cashOut(ddwMoney, ddwPaperMoney, cookie, token) {
  return new Promise((resolve) => {
    $.get(taskUrl(`user/CashOut`, cookie, `ddwMoney=${ddwMoney}&ddwPaperMoney=${ddwPaperMoney}&strPgUUNum=${token['farm_jstoken']}&strPgtimestamp=${token['timestamp']}&strPhoneID=${token['phoneid']}`), (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
...
// 升级建筑
function getBuildInfo(cookie, body) {
  return new Promise((resolve) => {
    $.get(taskUrl(`user/GetBuildInfo`, cookie, body), (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
...
function buildLvlUp(cookie, body) {
  return new Promise((resolve) => {
    $.get(taskUrl(`user/BuildLvlUp`, cookie, body), (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
...
// 获取用户信息
function getUserInfo(cookie, info, index, UserName, showInvite = true) {
  return new Promise(async (resolve) => {
    $.get(taskUrl(`user/QueryUserInfo`, cookie), (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
...
function taskUrl(function_path, cookie, body = '') {
...
function showMsg(index, nickName, UserName) {
  return new Promise(resolve => {
