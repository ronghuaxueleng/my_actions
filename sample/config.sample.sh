## Version: v1.00.0
## Date: 2021-09-01
## Update Content: 全新版本，全新出发

## 上面版本号中，如果第2位数字有变化，那么代表增加了新的参数，如果只有第3位数字有变化，仅代表更新了注释，没有增加新的参数，可更新可不更新

## ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓ 自 定 义 环 境 变 量 设 置 区 域 ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
## 可在下方编写您需要用到的额外环境变量，具体查看脚本内的注释，格式：export 变量名="变量值"
...
## 下面是获取方式，有效期均为 1 个月有效期
## 1. 通过控制面板主页的 "扫码获取Cookie" 获取
## 2. 通过浏览器开发工具获取 https://github.com/chinnkarahoi/jd_scripts/blob/master/backUp/GetJdCookie.md
## 3. 通过浏览器插件获取 https://github.com/chinnkarahoi/jd_scripts/blob/master/backUp/GetJdCookie2.md
...
################################## 账号临时屏蔽（选填） ##################################
## 如果某些 Cookie 已经失效了但暂时还没法更新，可以使用此功能在不做任何更改的前提下临时屏蔽掉某些编号的 Cookie，在指定执行中屏蔽设置不会生效
## 全局屏蔽 Cookie，举例：TempBlockCookie="2 4"  临时屏蔽掉Cookie2和Cookie4
TempBlockCookie=""

## 如果只是想要屏蔽某个账号不玩某些小游戏，可以参考下面 case 这个命令的例子来控制，脚本名称请去掉后缀 “.js”，同时注意代码缩进
## 实际执行时需精确执行才能生效，例如您屏蔽的活动是东东农场并且 case 例子中填写的是 jd_fruit，那么执行 task jd_fruit now 才能生效
## 反之如果执行 task jd_fruit.js now 或者 task fruit now 是不生效的，建议活动名称跟定时任务中的名称保持一致
# case $1 in
# jd_fruit)
...
## 方式二：只下载想要的文件（基于 Wget）
## 请先确认您能正常下载该raw文件才列在下方，无论是 Github 还是 Gitee 的仓库，请只填入 raw 原始文件链接。
## 注意缩进和格式，每行开头四个空格，一行一个文件链接，首尾一对半角括号，示例：
## OwnRawFile=(
...
## "AutoAddOwnCron": 自动增加；"AutoDelOwnCron": 自动删除；如需启用请设置为 "true"，否则请设置为 "false"，默认均为 "true"
## 本shell脚本不一定能完全从脚本中识别出有效的cron设置，如果发现不能满足您的需要，请设置为 "false" 以取消自动增加或自动删除。
AutoAddOwnCron="true"
AutoDelOwnCron="true"
...
## 具体效果查看效果图：https://github.com/chinnkarahoi/jd_scripts/blob/master/icon/bean_sign_simple.jpg
...
################################## 定义自动互助功能（选填） ##################################
## 如想在运行互助类活动脚本时直接从 task exsc 中自动获取互助码并进行互助，请将 AutoHelpOther 设置为 true
## 工作原理为导入最新的导出互助码日志，当 AutoHelpOther 设置为 true 时会导入最新的导出互助码日志
