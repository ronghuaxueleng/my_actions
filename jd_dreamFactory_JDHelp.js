...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `http://www.jdhelp.cf/jdcodes/getcode.php?type=jxfactory&num=${randomCount}`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function submitCode() {
    return new Promise(async resolve => {
    $.get({url: `http://www.jdhelp.cf/jdcodes/submit.php?code=${myInviteCode}&type=jxfactory`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    ...
    try{readShareCodeRes = {"code": -1};}catch(e){}
