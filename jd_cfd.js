    ...
    $.get(
      ...
      async (err, resp, data) => {
        try {
          const {
            iRet,
            sData: { Sign = [{}], dwUserFlag },
            sErrMsg,
          } = JSON.parse(data);
          $.log(`\n签到列表：${sErrMsg}\n${$.showLog ? data : ""}`);
          const [{ dwStatus, ddwMoney }] = Sign.filter(
            (x) => x.dwShowFlag === 1
          );
          if (dwStatus === 0) {
  ...
  return new Promise(async (resolve) => {
    ...
    for (var _key of Object.keys($.info.SceneList)) {
      try {
        ...
        //领取普通助力的财富
        const employeeList = eval(
          "(" + JSON.stringify(sceneList[_key].EmployeeList) + ")"
        );
        if (employeeList !== "") {
...
function getMoney_dwSource_1(_key, sceneList) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        `user/GetMoney`,
        `ptag=&dwSceneId=${_key}&strEmployeeId=undefined&dwSource=1`,
        `_cfd_t,bizCode,dwEnv,dwSceneId,dwSource,ptag,source,strEmployeeId,strZone`
      ),
      async (err, resp, data) => {
        try {
          const { iRet, dwMoney, sErrMsg } = JSON.parse(data);
          $.log(
            `\n【${sceneList[_key].strSceneName}】🏝岛主 : ${
              sErrMsg == "success" ? `获取财富值：¥ ${dwMoney || 0}` : sErrMsg
            } \n${$.showLog ? data : ""}`
          );
        } catch (e) {
          $.logErr(e, resp);
...
function getMoney_dwSource_2(_key, sceneList, key) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        `user/GetMoney`,
        `ptag=&dwSceneId=${_key}&strEmployeeId=${key}&dwSource=2`,
        `_cfd_t,bizCode,dwEnv,dwSceneId,dwSource,ptag,source,strEmployeeId,strZone`
      ),
      async (err, resp, data) => {
        try {
          ...
          $.log(
            `\n【${sceneList[_key].strSceneName}】👬好友: ${
              sErrMsg == "success"
                ? `获取普通助力财富值：¥ ${dwMoney || 0}`
                : sErrMsg
            } \n${$.showLog ? data : ""}`
...
function getMoney_dwSource_3(_key, sceneList) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        `user/GetMoney`,
        `ptag=&dwSceneId=${_key}&strEmployeeId=&dwSource=3&strPgtimestamp=${$.strPgtimestamp}&strPhoneID=${$.strPhoneID}&strPgUUNum=${$.strPgUUNum}`,
        ``
      ),
      async (err, resp, data) => {
        try {
...
function getAdvEmployee(_key) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        `user/GetAdvEmployee`,
        `ptag=&dwSenceId=${_key}&dwIsSlave=0`,
        `_cfd_t,bizCode,dwEnv,dwIsSlave,dwSenceId,ptag,source,strZone`
      ),
      async (err, resp, data) => {
        try {
          const {
            SceneEmployeeInfo: { SceneId, SceneName, dwCurStage },
            dwNextSceneId,
            sErrMsg,
          } = JSON.parse(data);
  ...
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        ...
        `dwSenceId=${_key}`,
        `_cfd_t,bizCode,dwEnv,dwSenceId,ptag,source,strZone`
      ),
      async (err, resp, data) => {
        try {
          const {
            sErrMsg,
            strAwardDetail: { strName },
          } = JSON.parse(data);
          $.log(
            `\n【${strSceneName}】💰雇主奖励：${
              sErrMsg == "success" ? `获取雇主奖励：¥ ${strName || 0}` : sErrMsg
            } \n${$.showLog ? data : ""}`
          );
        } catch (e) {
          $.logErr(e, resp);
...
function friendCircle() {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        `user/FriendCircle`,
        `ptag=&dwPageIndex=1&dwPageSize=20`,
        `_cfd_t,bizCode,dwEnv,dwPageIndex,dwPageSize,ptag,source,strZone`
      ),
      async (err, resp, data) => {
        try {
          //$.log(`\n好友圈列表\n${data}`);
...
function queryFriendIsland(strShareId) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        `user/QueryFriendIsland`,
        `ptag=7155.9.47&strShareId=${strShareId}&sceneval=2`,
        `_cfd_t,bizCode,ddwTaskId,dwEnv,ptag,source,strShareId,strZone`
      ),
      async (err, resp, data) => {
        try {
          ...
          if (sErrMsg === "success") {
            ...
            for (sceneId of sceneIds) {
              await stealMoney(
                ...
                sceneId,
                strFriendNick,
                sceneList[sceneId].strSceneName
              );
              await $.wait(500);
            }
          }
...
function stealMoney(strShareId, sceneId, strFriendNick, strSceneName) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        `user/StealMoney`,
        `ptag=&strFriendId=${strShareId}&dwSceneId=${sceneId}&sceneval=2`,
        `_cfd_t,bizCode,dwEnv,dwSceneId,ptag,source,strFriendId,strZone`
      ),
      async (err, resp, data) => {
        try {
          //$.log(data);
          ...
          $.log(
            `\n🤏偷取好友【${strFriendNick}】【${strSceneName}】财富值：¥ ${
              dwGetMoney ? dwGetMoney : sErrMsg
            }\n${$.showLog ? data : ""}`
          );
        } catch (e) {
          $.logErr(e, resp);
...
function doTreasureHunt(place) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        `consume/TreasureHunt`,
        `ptag=7155.9.47&strIndex=${place}&dwIsShare=0`,
        `_cfd_t,bizCode,dwEnv,dwIsShare,ptag,source,strIndex,strZone`
      ),
      async (err, resp, data) => {
        try {
          //$.log(data);
    ...
    if (userStatusConfigExtra.length > 0) {
      ...
      for (let i = 1; i <= configExtra; i++) {
        await $.get(
          taskListUrl(
            `DoTask`,
            `ptag=&taskId=${taskId}&configExtra=${configExtra}`,
  ...
  return new Promise(async (resolve) => {
    switch (taskType) {
          ...
          async (err, resp, data) => {
            try {
              ...
              if (msg.indexOf("活动太火爆了") !== -1) {
                ...
                str =
                  msg + prizeInfo
                    ? ` 获得财富值 ¥ ${JSON.parse(prizeInfo).ddwMoney}`
                    : "";
              }
              $.log(
                `\n${taskName}【领日常奖励】：${str}\n${$.showLog ? data : ""}`
      ...
      case 1: //成就奖励
        ...
        $.get(
          taskUrl(
            `consume/AchieveAward`,
            `ptag=&strTaskIndex=${strTaskIndex}`,
            `_cfd_t,bizCode,dwEnv,ptag,source,strZone,strTaskIndex`
          ),
          async (err, resp, data) => {
            try {
...
function funCenterState() {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(`consume/FunCenterState`, `strType=1`),
      async (err, resp, data) => {
        try {
          const {
            SlotMachine: {
...
function soltMachine(strCouponPool, strGoodsPool, ddwConfVersion) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        `consume/SlotMachine`,
        `strCouponPool=${strCouponPool}&strGoodsPool=${strGoodsPool}&ddwConfVersion=${ddwConfVersion}`
      ),
      async (err, resp, data) => {
        try {
          ...
          $.log(
            `\n【抽奖结果】🎰 ${
              strAwardPoolName != "" ? "未中奖" : strAwardPoolName
            } \n${$.showLog ? data : ""}`
          );
        } catch (e) {
          $.logErr(e, resp);
      ...
      async (err, resp, _data) => {
        try {
          ...
          $.get(
            taskUrl(
              "user/JoinScene",
              `ptag=&strPgtimestamp=${$.strPgtimestamp}&strPhoneID=${
                $.strPhoneID
              }&strPgUUNum=${$.strPgUUNum}&strShareId=${escape(
                data.value
              )}&dwSceneId=${sceneId}&dwType=2`,
              `_cfd_t,bizCode,dwEnv,dwSceneId,dwType,ptag,source,strPgUUNum,strPgtimestamp,strPhoneID,strShareId,strZone`
            ),
            async (err, resp, data) => {
              try {
                const { sErrMsg, data: { rewardMoney = 0 } = {} } =
                  JSON.parse(data);
                $.log(
                  `\n【👫🏻超级助力】超级助力：${sErrMsg}\n${
                    $.showLog ? data : ""
      ...
      async (err, resp, _data) => {
        try {
          ...
          $.get(
            taskUrl(
              "user/JoinScene",
              `strShareId=${escape(data.value)}&dwSceneId=${sceneId}`
            ),
            async (err, resp, data) => {
              try {
                const { sErrMsg, data: { rewardMoney = 0 } = {} } =
    ...
    $.get(taskUrl(`user/GatherForture`), async (err, resp, g_data) => {
      try {
        ...
        if (!strGroupId) {
          ...
          $.post(
            {
              url: `https://api.ninesix.cc/api/jx-cfd-group/${strGroupId}/${encodeURIComponent(
                strPin
              )}`,
            },
            async (err, resp, _data) => {
...
function openGroup() {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        `user/OpenGroup`,
        `dwIsNewUser=${$.info.dwIsNewUser}`,
        `_cfd_t,bizCode,dwEnv,dwIsNewUser,ptag,source,strZone`
      ),
      async (err, resp, data) => {
        try {
      ...
      try {
        ...
        for (var i = 0; i < PeriodBox.length; i++) {
          ...
          if (dwStatus == 2) {
            await $.wait(1000);
            await $.get(
              taskUrl(`user/OpenPeriodBox`, `dwSeq=${dwSeq}`),
              async (err, resp, data) => {
                try {
                  const { dwMoney, iRet, sErrMsg } = JSON.parse(data);
                  $.log(
                    `\n【🏝寻宝大作战】【${strBrandName}】开宝箱：${
                      sErrMsg == "success"
                        ? ` 获得财富值 ¥ ${dwMoney}`
                        : sErrMsg
                    }\n${$.showLog ? data : ""}`
  ...
  Object.keys(l).forEach((e) => {
    if (new RegExp("(".concat(e, ")")).test(d)) {
      ...
      d = d.replace(
        ...
        1 == RegExp.$1.length
          ? l[e]
          : "".concat(a).concat(l[e]).substr("".concat(l[e]).length)
      );
    }
  });
  return d;
...
function decrypt(time, stk, type, url) {
  ...
  if (stk) {
    ...
    stk.split(",").map((item, index) => {
      st += `${item}:${getUrlQueryParams(url, item)}${
