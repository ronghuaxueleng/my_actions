...
活动名称：抢双11无门槛红包
更新时间：2021-10-28
活动地址：https://u.jd.com/3t9LXZe
仅CK1助力作者其余助力CK1
默认使用作者提供的京粉链接，如需使用自己的请定义环境变量 export JX_CODE="你的code"
cron 0 10,12,20 * * * jd_redEnvelope.js
*/
const $ = new Env("京享红包");
...
async function main() {
    ...
    $.max = false;
    $.hotFlag = false;
    const jxCodeArr = ["yI1HjQO", "yL1RvXw", "yI1xAQs", "yK1cFr7", "yL1kfJx"];
    let jxCode = jxCodeArr[Math.floor(Math.random() * jxCodeArr.length)];
    let JX_CODE = $.isNode() ? process.env.JX_CODE ? process.env.JX_CODE : '' : $.getdata("JX_CODE") ? $.getdata("JX_CODE") : ''
    if (JX_CODE) {
        if ($.index === 1) {
            jxCode = JX_CODE;
        } else {
            jxCode = [jxCode, JX_CODE];
            jxCode = jxCode[Math.floor(Math.random() * jxCode.length)];
        }
    }
