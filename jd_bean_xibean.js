 ...
 */
const $ = new Env('过期京豆自动兑换喜豆');
const notify = $.isNode() ? require('./sendNotify') : '';
//Node.js用户请在jdCookie.js处填写京东ck;
const jdCookieNode = $.isNode() ? require('./jdCookie') : '';
const jdNotify = process.env.XIBEAN_NOTIFY ? process.env.XIBEAN_NOTIFY === 'true' : false;  // 是否推送通知，默认不推送
let allMessage = '';
...
!(async () => {
  ...
  if (jdNotify && allMessage) {
    if ($.isNode()) {
      ...
      $.msg($.name, '', allMessage , {"open-url": "https://bean.m.jd.com/beanDetail/index.action?resourceValue=bean"});
    }
  }
})()
    .catch((e) => {
...
function getXidouCount() {
  return new Promise(async resolve => {
    const options = {
      "url": `https://m.jingxi.com/activeapi/querybeanamount?_=${Date.now()}&sceneval=2&g_login_type=1&callback=jsonpCBKA&g_ty=ls`,
      "headers": {
        "Accept": "*/*",
        "Accept-Encoding": "gzip, deflate, br",
        "Accept-Language": "zh-cn",
    ...
    $.get(options, (err, resp, data) => {
      try {
        if (err) {
          ...
          if (data) {
            // console.log(data)
            data = getJsonpData('jsonpCBKA', data);
            // console.log(data)
            if (data.errcode === 0) {
              $.xibeanCount = data.data.xibean;
              console.log(`当前喜豆：${$.xibeanCount}`);
            }
          } else {
            console.log(`京东服务器返回空数据`)
          }
